...
const fs = require("fs"); // Allows the command handler to work!
...
client.on("ready", () => {
  ...
  function UpdateStatus(text, x, y) {
      ...
      UpdateStatus(("\x1b[47m\x1b[31mWelcome " + client.user.tag + " || Id: " + client.user.id + " || Guilds: " + client.guilds.size + " || \x1b[47m\x1b[31mDiscord API Version " + Discord.version + " || Connecting || Connected || Ready ||"), 0, process.stdout.rows)
        ...
        client.user.setActivity("Do /help ", {type: "PLAYING"});
    });
...
client.on("guildCreate", async guild => {
  let guildCreateChannel = client.channels.get("457627003718467584");

  let general = guild.channels.find('name', 'bot-joined');
...
client.on("guildDelete", async guild => {
  let guildCreateDelete = client.channels.get("457627003718467584");
